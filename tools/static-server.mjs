#!/usr/bin/env node
/**
 * Wrapper para `python3 -m http.server <port> -d dist` que:
 ...
 *
 * Uso direto: PORT=4399 node tools/static-server.mjs
 * Pelo Playwright: `command: 'node tools/static-server.mjs'` com `port: 0`
...
const child = spawn(
  'python3',
  ['-m', 'http.server', String(port), '--bind', HOST, '-d', 'dist'],
  { stdio: 'inherit' }
);
